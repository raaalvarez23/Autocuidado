...
export default function Home() {
  ...
  async function loadCars() {
    ...
    const withStatus = await Promise.all(carsData.map(async car => {
      ...
      return { ...car, records: records || [], vencidas: statuses.filter(s => s.status === 'danger').length, proximas: statuses.filter(s => s.status === 'warn').length, statuses }
    }))
    setCars(withStatus)
    setLoading(false)
  }
    ...
    <div className="app-shell">
      ...
      <div className="main-content">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>¡Hola, {user?.name}! 👋</h2>
            <p style={{ color: 'var(--text-sec)', fontSize: 12, marginTop: 2 }}>Mantén tus autos en perfecto estado</p>
          </div>
        </div>
        ...
        {criticalCar && criticalStatus && (
          <Link to={`/cars/${criticalCar.id}`} style={{ textDecoration: 'none' }}>
            <div style={{ background: 'var(--surface)', border: '1px solid #fecaca', borderRadius: 10, padding: '12px 14px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 14, height: 14, borderRadius: '50%', background: '#ef4444', flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{criticalStatus.partName}</div>
                <div style={{ fontSize: 11, color: 'var(--text-sec)', marginTop: 2 }}>
                  {criticalCar.name} · Te pasaste por <strong style={{ color: 'var(--text)' }}>{Math.abs(criticalStatus.remaining).toLocaleString('es-CL')} km</strong>
                </div>
                <div style={{ fontSize: 11, color: 'var(--text-sec)' }}>Próxima a los {criticalStatus.nextKm?.toLocaleString('es-CL')} km</div>
          ...
          <Link key={car.id} to={`/cars/${car.id}`} className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-sec)', marginTop: 2 }}>{car.brand} {car.model} · {car.plate}</div>
              ...
              <div style={{ borderTop: '1px solid var(--border-light)', paddingTop: 8 }}>
                {car.statuses.filter(s => s.status === 'danger').slice(0, 2).map(s => (
                  <div key={s.partName} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, color: 'var(--text-third)' }}>{s.partName}</span>
...
function ImportModal({ onClose, user, cars }) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (createNew) {
        const { data: newCar, error: carErr } = await supabase.from('cars').insert({ ...parsed.car, user_id: user.id, name: parsed.car.name + ' (importado)' }).select().single()
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        {!success && step === 'inicio' && (
          <>
            <p style={{ fontSize: 12, color: 'var(--text-sec)', marginBottom: 16 }}>Pide al dueño anterior que vaya al detalle de su auto → Compartir, y te dé el código.</p>
            ...
            <button className="btn btn-primary" onClick={handleParse} disabled={!code.trim()}>Ver historial e importar →</button>
        ...
        {!success && step === 'preview' && parsed && (
          <>
            <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{parsed.car.name}</div>
              <div style={{ fontSize: 12, color: 'var(--text-sec)' }}>{parsed.car.brand} {parsed.car.model} · {parsed.car.plate}</div>
              <div style={{ fontSize: 12, color: 'var(--text-sec)' }}>{parsed.records.length} registros de mantención</div>
            </div>
            <div className="form-group">
              <label className="form-label">¿Dónde importar?</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                <button onClick={() => setCreateNew(true)} style={{ flex: 1, padding: '8px', borderRadius: 8, border: `2px solid ${createNew ? '#2d4faa' : 'var(--border)'}`, background: createNew ? '#eff6ff' : 'var(--surface)', color: createNew ? '#2d4faa' : 'var(--text)', fontSize: 12, cursor: 'pointer' }}>
                  ➕ Crear auto nuevo
                </button>
                <button onClick={() => setCreateNew(false)} style={{ flex: 1, padding: '8px', borderRadius: 8, border: `2px solid ${!createNew ? '#2d4faa' : 'var(--border)'}`, background: !createNew ? '#eff6ff' : 'var(--surface)', color: !createNew ? '#2d4faa' : 'var(--text)', fontSize: 12, cursor: 'pointer' }}>
                  🚗 Auto existente
                </button>
              </div>
              {!createNew && (
            ...
            <div style={{ maxHeight: 150, overflowY: 'auto', marginBottom: 12 }}>
              {parsed.records.map((r, i) => (
                <div key={i} style={{ fontSize: 12, padding: '5px 0', borderBottom: '1px solid var(--border-light)', color: 'var(--text-third)' }}>
